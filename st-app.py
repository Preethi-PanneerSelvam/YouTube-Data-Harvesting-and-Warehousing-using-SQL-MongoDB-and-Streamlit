...
# Google API 
api_key = "AIzaSyAo7PKVQxDlM0qzV-D0I42f_eMggfmE-pY"
...
def get_comment_details(vid_id):
    comment_data = []
    try:
        next_page_token = None
        while True:
...
#streamlit
st.set_page_config(page_title = "Youtube Data Harvesting and Warehousing | created by Preethi",
                   layout = 'wide',
                   initial_sidebar_state ='expanded',
                   menu_items = {'About' : 'Created by Preethi'})
...
with st.sidebar:
    selected = option_menu(None, ["Home","Extract and Transform","View"], 
                           ...
                           styles={"nav-link": {"font-size": "15px", "text-align": "centre", "margin": "0px", 
                                   ...
                                   "nav-link-selected": {"background-color": "#7fd104"}})
...
if selected == "Home":
    ...
    col1.markdown("#### :green[Technologies used] : Youtube Data API, Python, MongoDB, MySql, Streamlit")
    col1.markdown("#### :green[Overview] : Retrieving the Youtube channels data from the Google API, storing it in MongoDB as datalake, migrating data into SQL database,and processing the data & displaying it in the Streamlit app.")
    col2.markdown("#   ")
    col2.markdown("#   ")
    col2.image("youtube.jpeg")
...
#Extract Tab
    with tab1:
         ...
         ch_id = st.text_input("Hint : Goto channel's home page > Right click >Click View page source > Find channel_id(ctrl+f)").split(',')
         ...
         if st.button("Upload to MongoDB"):
            with st.spinner('loading...'):
                ...
                collection3.insert_many(comm_details)
    ...
    with tab2:     
        ...
        if st.button("Submit"):
            try:
                ...
                st.error(f"4Error:'{err}'")

# VIEW PAGE
if selected == "View":
    
    st.write("## :green[Select Questions]")
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        mycursor.execute("""SELECT channels.channel_name AS Channel_Name, COUNT(*) AS Comment_Count, title AS Video_Name 
                            FROM videos JOIN comments ON videos.video_id = comments.video_id
                            JOIN channels ON videos.Channel_Id = channels.Channel_Id GROUP BY videos.Video_Id, videos.title, channels.Channel_Name ORDER BY Comment_Count DESC""")
        df = pd.DataFrame(mycursor.fetchall(),columns=mycursor.column_names)  
        st.write(df)
        ...
        mycursor.execute("""SELECT channel_name AS Channel_Name,title AS Video_Name,comments AS Comments
                            FROM videos
